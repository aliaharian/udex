...
class ConnectController extends Controller
{
    ...
    public function connect_service_store(Request $request)
    {
        ...
        /* Upload File */
        $files = $request->file('upload_your_architectural_drawings');
//        dd($files);
        if ($request->hasFile('upload_your_architectural_drawings')) {
            foreach ($files as $file) {
                if ($path = $file->store('connect-service/file')) {
                    ...
                    $Attachments->type = $file->extension();
        ...
        foreach ($request->service_are_you_looking as $serviceLooking){
            $Connect = new Connect;
            $Connect->uid = $user_id;
            $ConnectMeta = [
                'service_are_you_looking' => $serviceLooking,
